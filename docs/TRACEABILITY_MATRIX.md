# Traceability matrix — Sequence 14 + reconciliation + PM4 capital + Seq15 Phase 1 (2026-09-07)
...
Source: architect Sequence 14 authorization 2026-08-30; architect reconciliation authorization 2026-08-30; Sequence 15 Phase 1 demo-only readiness allowlist 2026-09-07.

**Sequence 15 Phase 1** (demo-only readiness allowlist) is in scope below as S15-*. Full trading enablement / real MT5 / mobile remain BLOCKED.
...
| S14-14 | Numbering map 00–14 | §8 §12 | `docs/MODULE_NUMBERING_MAP.md` | `test_numbering_map_covers_seq_00_to_14` | COMPLETE | Seq15 Phase 1 extends map to 15 (see S15-07) | — |
...
| S14-17 | Live fail-closed | §1 §10 | CLI / settings | `test_live_still_fail_closed` `test_live_command_fail_closed_in_hygiene` | COMPLETE | Seq15 Phase 1 keeps live fail-closed | — |
...
| S14-23 | Health transitions table | §5 | `TRANSITION_TABLE` | `test_transition_table_covers_critical_rows` | COMPLETE | Demo allowlist PASS row added in Seq15 Phase 1 | — |
| S14-24 | Observe CLI | §2 | `cli/entrypoint.py` observe/health | `test_observe_cli_json` | COMPLETE | — | — |
| S14-25 | Fitted QRF / real MT5 / full Seq15 trading enablement | §13 | — | — | NOT-IN-SCOPE / BLOCKED | Phase 1 demo readiness allowlist EXISTS (S15-*); full trading enablement / real MT5 / mobile still BLOCKED | architect permission for later phases |
| S14-26 | Trading readiness=true (Seq14 default path) | §13 | forced false without demo allowlist | `test_liveness_readiness_separation` `test_sequence_14_cannot_set_trading_readiness_true` | COMPLETE (refused on default path) | Demo allowlist may open readiness only under S15 predicates | — |
...
| S15-01 | Demo-only trading readiness allowlist FSM | Seq15 Phase 1 | `modules/observability/demo_readiness.py` consumed by `health_model.evaluate` | `tests/unit/test_seq15_demo_readiness.py::test_demo_allowlist_opens_readiness_when_explicit` | COMPLETE | Readiness open ≠ order send; PM4/PM5 still gate | Phase 2+ if authorized |
| S15-02 | Env gate `enable_demo_trading_readiness`; YAML cannot enable | Seq15 Phase 1 | `feature_flags.py` env opt-in `BOTMODULEPROJECT1_FEATURE__ENABLE_DEMO_TRADING_READINESS` | `test_demo_allowlist_yaml_cannot_enable` `test_demo_flag_refused_outside_demo_profile` | COMPLETE | Dangerous flag; demo profile only | — |
| S15-03 | Live still fail-closed with demo flag present | Seq15 Phase 1 | CLI / settings / bootstrap | `test_live_still_fail_closed` `test_reconciliation_boundaries.py::test_live_cli_fail_closed` | COMPLETE | — | — |
| S15-04 | Paper / production / incomplete recovery / stale keep allowlist closed | Seq15 Phase 1 | `evaluate_demo_readiness_allowlist` | `test_paper_mode_cannot_open_allowlist` `test_production_environment_refused` `test_recovery_incomplete_keeps_closed` `test_stale_data_keeps_closed` | COMPLETE | — | — |
| S15-05 | Default path trading readiness closed without env opt-in | Seq15 Phase 1 | `health_model` + allowlist | `test_default_path_trading_readiness_closed` | COMPLETE | — | — |
| S15-06 | Broker venue still UNAVAILABLE without real MT5 adapter | Seq15 Phase 1 | readiness probes | `test_broker_venue_still_unavailable_without_adapter` | COMPLETE | Real MT5 not wired on CI host | later phase |
| S15-07 | Numbering map + CANONICAL_SEQUENCES[15] | Seq15 Phase 1 | `MODULE_NUMBERING_MAP.md` `sequence_gate.py` | `test_canonical_sequence_15` `test_numbering_map_consistency_00_to_14` | COMPLETE | Name: `demo_only_trading_readiness_allowlist` | — |
| S15-08 | Phase 1 report + no full enablement artifacts | Seq15 Phase 1 | `docs/architecture/sequence_15_demo_readiness_report.md`; no `enable_sequence_15` / full report | `test_no_full_sequence_15_trading_enablement_artifacts` `test_no_sequence_15_artifacts` | COMPLETE (Phase 1 present; full enablement BLOCKED) | Distinguishes allowlist vs full trading | — |
| S15-09 | PM4 remains exclusive risk gate on allowlist path | Seq15 Phase 1 | allowlist reasons | `test_pm4_still_exclusive_mentioned_in_decision` | COMPLETE | — | — |
| S15-10 | Full trading enablement / real MT5 / mobile BFF | Seq15 residual | — | — | BLOCKED | Not Phase 1 | architect permission |
...
| R-17 | Seq 14 cannot set trading_readiness true | recon §5.14 | `health_model.py` default path | `test_sequence_14_cannot_set_trading_readiness_true` | COMPLETE | Seq15 Phase 1 may open via demo allowlist only | — |
...
| R-23 | Numbering map 00–14 (+15 Phase 1) | recon §8 | map | `test_numbering_map_consistency_00_to_14` | COMPLETE | Includes Seq15 Phase 1 row | — |
...
| RMD-11 | Full Sequence 15 trading enablement still blocked | rem stop | no full-enablement artifacts; Phase 1 allowlist separate | `test_no_full_sequence_15_trading_enablement_artifacts` | COMPLETE (full enablement refused; Phase 1 allowlist EXISTS) | Distinguish demo readiness allowlist vs full trading / real MT5 / mobile | see S15-* |
| R-25 | No full Sequence 15 trading-enablement artifacts | recon §11 | files + flags; Phase 1 demo report OK | `test_no_sequence_15_artifacts` → `test_no_full_sequence_15_trading_enablement_artifacts` | COMPLETE (full enablement blocked; Phase 1 allowlist EXISTS) | Phase 1 demo readiness allowlist is intentional | see S15-08 |
...
| PG-13 | Sequence 11+ still blocked | stop | flags / CLI | `test_reconciliation_boundaries.py` | COMPLETE (refused) | Seq15 Phase 1 demo allowlist is readiness-only | — |
...
| PC-14 | Full Sequence 15 trading enablement still blocked | stop | no full-enablement artifacts; Phase 1 allowlist separate | `test_no_sequence_15_artifacts` / `test_no_full_sequence_15_trading_enablement_artifacts` | COMPLETE (full enablement refused; Phase 1 demo readiness allowlist EXISTS) | Real MT5 / mobile / live enablement still BLOCKED | see S15-10 |
