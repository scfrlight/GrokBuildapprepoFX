...
the PM3-Strategy Engine in Sequence 04, PM3 forecasting / QRF in Sequence 05,
and the PM4 Risk Gate in Sequence 06:

- `settings.py` — pydantic-settings, `BOTMODULEPROJECT1_` prefix, fingerprint, `pm2`, `pm3_strategy_engine`, `pm3_forecasting`, and `pm4_risk_gate` sections
...
binds `PM3ForecastingModule` instead of `NullModel`. When `enable_pm4_risk_gate`
is env-opted in test/research, it binds `PM4RiskGateModule` instead of
`NullRiskGate`. Otherwise the placeholders stay. No flag creates execution
capability.

Must not contain strategy math as the composition root, risk sizing as the
composition root, or broker calls.
The PM3-Strategy Engine lives in `modules/pm3_strategy_engine/`.
PM3 forecasting / QRF lives in `modules/pm3_forecasting/`.
PM4 lives in `modules/pm4_risk_gate/` (registry name `pm4_risk`).
