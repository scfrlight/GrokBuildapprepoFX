...
from botmoduleproject1.contracts.v1.observability import (
    ALLOWED_LOG_SYMBOLS, ErrorCode, ProbeState, REQUIRED_LOG_FIELDS,
)
...
from botmoduleproject1.modules.observability.metrics import (
    CATALOG_BY_NAME, METRIC_CATALOG, CardinalityError, MetricRegistry, UnknownMetricError,
)
from botmoduleproject1.modules.observability.module import ObservabilityModule
from botmoduleproject1.modules.observability.redaction import contains_forbidden_secret, redact_mapping
...
def test_structured_log_schema_and_utc():
    event = emit_event(
        event_name="health.evaluated", module="observability", sequence=14, profile="test",
        status="degraded", actor="doctor", symbol="EURUSD", error_code=ErrorCode.STALE_DATA_ERROR,
        correlation_id=uuid4(), causation_id=uuid4(), trace_id=uuid4(), metadata={"note": "ok"},
    )
...
def test_naive_timestamp_rejected():
    with pytest.raises((LogSchemaError, ValueError)):
        emit_event(event_name="x", module="observability", sequence=14, profile="test", status="ok", timestamp=datetime.now())

def test_unknown_symbol_rejected():
    with pytest.raises(LogSchemaError):
        emit_event(event_name="x", module="observability", sequence=14, profile="test", status="ok", symbol="DOGEUSD")

def test_correlation_causation_propagation():
    corr, trace = uuid4(), uuid4()
    first = emit_event(event_name="a", module="observability", sequence=14, profile="test", status="ok", correlation_id=corr, trace_id=trace)
    second = emit_event(event_name="b", module="observability", sequence=14, profile="test", status="ok", correlation_id=first.correlation_id, causation_id=first.correlation_id, trace_id=first.trace_id)
    assert second.correlation_id == corr and second.causation_id == first.correlation_id and second.trace_id == trace

def test_secret_redaction_in_log_metadata():
    secret = "super-secret-token-value-12345"
    event = emit_event(
        event_name="leak-attempt", module="observability", sequence=14, profile="test", status="ok",
        metadata={"password": secret, "telegram_bot_token": secret, "note": "safe", "dsn": "postgres://user:hunter2@localhost/db"},
    )
    blob = json.dumps(event_as_log_dict(event))
    assert secret not in blob and "hunter2" not in blob
    assert event.metadata["password"] == "present" and event.metadata["note"] == "safe"
...
def test_redact_mapping_nested():
    ...
    assert out["a"]["api_key"] == "present" and out["a"]["ok"] == 1 and out["list"][0]["token"] == "present"

def test_metric_catalog_covers_required_names():
    names = {s.name for s in METRIC_CATALOG}
    required = {
        "botmodule.persistence.latency_ms", "botmodule.persistence.errors", "botmodule.outbox.backlog",
        "botmodule.outbox.relay_lag_ms", "botmodule.inbox.dedupe_hits", "botmodule.dead_letter.count",
        "botmodule.projection.lag", "botmodule.projection.rebuild_duration_ms", "botmodule.replay.duration_ms",
        "botmodule.snapshot.age_seconds", "botmodule.checkpoint.age_seconds",
        "botmodule.reconciliation.mismatch_count", "botmodule.reconciliation.degraded_count",
        "botmodule.risk.denials", "botmodule.execution.simulation_attempts", "botmodule.execution.duplicate_attempts",
        "botmodule.retry.count", "botmodule.market.stale_events", "botmodule.incidents.active",
        "botmodule.incidents.unresolved", "botmodule.operator.actions", "botmodule.operator.denied_actions",
        "botmodule.health.transitions", "botmodule.shutdown.duration_ms", "botmodule.recovery.duration_ms",
    }
    assert required <= names
    for spec in METRIC_CATALOG:
        assert spec.safe_default == 0.0 and "bounded" in spec.cardinality
...
def test_metric_cardinality_limits():
    ...
    assert reg.inc("botmodule.risk.denials", module="pm4_risk_gate", outcome="denied").value == 1.0

def test_liveness_readiness_separation():
    health, ready = evaluate(_settings(), lifecycle=LifecycleState.DEGRADED)
    assert ready.process_alive and ready.accept_observe and not ready.accept_trade
    assert health.trading_readiness is False
    assert ready.liveness is ProbeState.PASS and ready.trading_readiness is ProbeState.FAIL
    assert ready.broker_venue is ProbeState.UNAVAILABLE

def test_venue_absent_is_not_pass():
    _h, ready = evaluate(_settings(), lifecycle=LifecycleState.DEGRADED)
    assert ready.broker_venue is ProbeState.UNAVAILABLE and ready.broker_venue is not ProbeState.PASS

def test_recovery_incomplete_not_trade_ready():
    health, ready = evaluate(_settings(), lifecycle=LifecycleState.WIRED)
    assert health.recovery_complete is False and health.trading_readiness is False
    assert ready.recovery_readiness is ProbeState.DEGRADED

def test_stale_data_safe_stop():
    health, ready = evaluate(_settings(), lifecycle=LifecycleState.DEGRADED, stale_data=True)
    assert health.stale_data and health.trading_readiness is False and ready.accept_trade is False
    assert any("stale" in r for r in health.reasons)

def test_integrity_fail_persistence_not_ready():
    health, ready = evaluate(_settings(), lifecycle=LifecycleState.DEGRADED, integrity_ok=False, persistence_enabled=True)
    assert ready.persistence_readiness is ProbeState.FAIL and health.trading_readiness is False
...
def test_all_flags_off_trading_false():
    ...
    assert health.flags_any_on is False and health.trading_readiness is False and ready.accept_trade is False

def test_transition_table_covers_critical_rows():
    keys = {(row[0], row[1]) for row in TRANSITION_TABLE}
    assert ("trading_readiness", "default_fail_closed") in keys
    assert ("trading_readiness", "demo_allowlist_explicit") in keys
    assert ("trading_readiness", "paper_mode") in keys
    ...
    for _dim, _cond, state, halt in TRANSITION_TABLE:
        if _dim == "trading_readiness" and _cond == "demo_allowlist_explicit":
            assert state is ProbeState.PASS and halt is False
        elif _dim == "trading_readiness":
            assert state is ProbeState.FAIL and halt is True
...
def test_error_taxonomy_complete_and_public_safe():
    ...
    assert ErrorCode.UNSAFE_OPERATION in halt_codes and ErrorCode.BROKER_UNAVAILABLE in halt_codes

def test_runbooks_have_twelve_fields_and_count():
    assert len(RUNBOOKS) == REQUIRED_RUNBOOK_COUNT
    required_ids = {
        "RB-STARTUP-CLEAN", "RB-SHUTDOWN-SAFE", "RB-STALE-MARKET-DATA", "RB-PERSISTENCE-UNAVAILABLE",
        "RB-LEDGER-INTEGRITY", "RB-OUTBOX-BACKLOG", "RB-DUPLICATE-CALLBACK", "RB-RECONCILIATION-DEGRADED",
        "RB-RECOVERY-AFTER-RESTART", "RB-BACKUP-VERIFICATION", "RB-RESTORE-VERIFICATION", "RB-FAILED-MIGRATION",
        "RB-FAILED-PROJECTION", "RB-MT5-UNAVAILABLE", "RB-SIM-EXEC-REJECTION", "RB-KILL-SWITCH",
        "RB-OPERATOR-PERMISSION-DENIAL", "RB-INCIDENT-ESCALATION", "RB-SECRET-EXPOSURE", "RB-CORRUPTED-EVIDENCE",
    }
    assert required_ids <= set(RUNBOOK_BY_ID)
    for rb in RUNBOOKS:
        assert rb.trigger and rb.symptoms and rb.safety_classification
        assert rb.automatic_system_behavior and rb.operator_inspection_commands and rb.prohibited_operator_actions
        ...
        for cmd in rb.operator_inspection_commands:
            ...
            for i, tok in enumerate(tokens):
                if tok.startswith(("tests/", "scripts/", "configs/", "docs/")):
                    path = tok.split("::", 1)[0]
...
def test_observability_module_bound_and_snapshot():
    ...
    assert snap.health.trading_readiness is False and snap.readiness.accept_trade is False
    assert snap.telegram_bound is False and snap.live_trading_enabled is False
    assert all(v is False for v in snap.flags.values())
    assert snap.metric_catalog_count == len(METRIC_CATALOG) and snap.runbook_count == 20
...
def test_observe_cli_json(capsys):
    code = main(["observe", "--profile", "test", "--config", str(TEST_YAML), "--json"])
    assert code == 0
